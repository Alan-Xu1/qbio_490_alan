...
rna_se <- GDCprepare(rna_query, directory = "/Users/alanxu/GDCdata")
...
is.na(rna_se@colData[1:15, "age_at_index"])
```
...
```{r Exercise 2.6}
write.csv(rna_clinical, "/Users/alanxu/Desktop/QBIO490/analysis_data/brca_rna_clincial_data.csv", row.names = TRUE)
```
...
```{r Exercise 3.4}
write.csv(rna_genes, "/Users/alanxu/Desktop/QBIO490/analysis_data/brca_rna_gene_data.csv", row.names = FALSE)
```
...
```{r Exercise 4.4}
write.csv(rna_counts, "/Users/alanxu/Desktop/QBIO490/analysis_data/brca_rna_count_data.csv", row.names = TRUE)
```
...
five_yr_death <- ifelse(rna_clinical$days_to_death == "NA", NA, ifelse(rna_clinical$days_to_death > 5*325.25, TRUE, FALSE))

# use boolean data from days_to_last_followup when five_yr_death is NA

five_yr_death_and_followup <- ifelse(is.na(five_yr_death), ifelse(rna_clinical$days_to_last_follow_up > 325.25*5, TRUE, FALSE), five_yr_death)
...
MUC16 appears to be lowly expressed in most patients. There is no definitive positive or negative 
correlation between the two genes. Furthur visualization is warranted.
----
...
Expression profile of MUC16 is more variable compared TP53. On average, MUC16 appears to be
more lowly expressed in patients. This indicates that MUC16 warrants furthur research
to investigate what causes the inherent wide expression profile variation among patients.
----
...
It appears that patients who survived past five years have higher expression of TP53 on average.
This observation is purely empirical at this point and P-value needs to be calculated in order
to quantitatively confirm the observation.
TP53 is implicated in tumor supression so it makes sense that higher expression of TP53 confers
longer survival time.
See: https://www.ncbi.nlm.nih.gov/gene/7157
----
...
There is no clear clustering or relationships between the chosen genes and clinical variables.
This make sense since the survival of a patient is unlikely to be determined by 4 genes but rather
on multiple variables. However, it does seem like MUC16 and MKI67 are generally lowly expressed in patients.
...
```{r Exercise 8.1}
# save files here
write.csv(geneABCD_counts, "/Users/alanxu/Desktop/QBIO490/analysis_data/ABCD.csv")
```
